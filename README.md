...
**502 や `connected:false` はセッション死の証拠ではない。** 同日に姉妹リポ crowd_cpp 側で、
学習中に kbridge が 502 と `connection reset` を返し「セッションが 1h44m で死んだ」と判断したが、
46 分後に同じ URL へ `POST /session` を投げたら同じカーネルに再接続でき、学習は
その間ずっと進んでいた（step 9500 → 15882）。落ちていたのはこちらの WebSocket だけだった。
**繋ぎ直してカーネルを見るまで、死んだと判断しないこと。**

監視の実測値: 195MB の `GET /download?raw=1` の最中でも `/healthz` は 5 ミリ秒で返る（大きい転送は
他の要求を塞がない）。一方 `/job` はカーネル往復があるので、box が学習で埋まっていると素で
15 秒前後かかる。**監視のタイムアウトは 60 秒以上**にしないと、忙しいだけのセッションを
死んだと誤判定する。
