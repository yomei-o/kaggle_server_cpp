...
留保 1: 対照（`--keepalive 0` で放置して死ぬのを見る）は測っていないので、
「idle タイマがそもそも 57 分より長い」可能性は原理的には残る。README の実測
30 分前後という前提の下では、57.5 分の生存はそれを超えている。

**留保 2（同日、姉妹リポ crowd_cpp 側の実測）: 502 と `connected:false` はセッション死の証拠ではない。**

学習を回している最中に kbridge が `/healthz` で `connected:false`、`/sh` で
`{"ok":false,"error":"connection reset"}`、いくつかの要求で HTTP 502 を返した。いったん
「セッションが 1 時間 44 分で死んだ」と判断したが、**間違いだった**。46 分後に同じ URL へ
`POST /session` を投げたら:

* 同じ `kernel_id` に再接続できた（`reuse: true`）
* 学習は止まっておらず、その間も進んでいた（step 9500 → 15882、GPU 使用率 100%）
* もう 1 本の学習はその間に完走していた

落ちていたのは**こちらの WebSocket だけ**。`execute()` は `ensure()` で張り直すが、その瞬間は
Kaggle のプロキシ自体が 502 を返していたので張り直しにも失敗し、エラーが表に出た形。

**教訓: 502 / connection reset を見たら、死んだと判断する前に `POST /session` で繋ぎ直して
カーネルを確認する。** 前日に crowd_cpp 側で「セッション落ちで学習 2 本を失った」と
記録された件も、同じ勘違いだった可能性がある。

あわせて測った監視の作法:

| 測ったこと | 結果 |
|---|---|
| 195MB の `GET /download?raw=1` の最中に `/healthz` | **5 ミリ秒**で応答。大きい転送は他の要求を塞がない |
| 学習で 4 vCPU が飽和している間の `GET /job` | **素で 14.8 秒**、転送中は 20 秒でもタイムアウト |

`/job` はカーネル往復があるので box が忙しいと十数秒かかる。**短いタイムアウトでポーリングすると
「セッションが死んだ」に見える**（今日それで何度も誤判定した）。監視のタイムアウトは 60 秒以上に。
...
**1. 切断からの復帰を、呼ぶ側から区別できるようにしてほしい**（2026-08-21、crowd_cpp から）

Kaggle のプロキシが一時的に 502 を返すと kbridge は `connected:false` になり、以降の `/sh` は
`{"ok":false,"error":"connection reset"}` を返す。プロキシが復帰したあとも、こちらが
`POST /session` を投げるまでその状態のままだった（カーネルもジョブも生きていた）。

* 欲しいもの: 呼び出しの中で張り直しを数回試す（短いバックオフ）か、少なくとも応答で
  「セッションは生きている可能性がある → `POST /session` で再接続せよ」と分かること。
  いまの文言では**セッション死と区別がつかず**、実際に誤判定して復旧を 46 分遅らせた。
* 仕様に書いてあると助かること: `/job` などカーネル往復のある要求は、box が学習で
  埋まっていると 15 秒前後かかる（実測）。監視側のタイムアウトの目安が README にあるとよい。
