...
//
// これで足りることは 2026-08-20 に実機で確認した。本物の T4 x2 セッションで
// ブラウザのタブを閉じたまま 57.5 分放置し、`pass` 以外を流さずに生き残った
// （カーネル pid・VM uptime・名前空間まで同一。README の「keep-alive の効き方」）。
// 既定 240 秒は、ブラウザ側 DOM クリック回避策の間隔（4〜6 分）に合わせたもの。
// python 版 server.py の KeepAlive と同じ挙動にすること。
