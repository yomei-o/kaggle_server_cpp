...
int X509_STORE_add_cert(X509_STORE* store, X509* toAdd)
{
    ...
    /* --- kbridge patch: assert を外す ---------------------------------------
     * Windows のシステムルートストアには mbedTLS が解釈できない証明書
     * （未知の署名アルゴリズム等）が混ざっており、NDEBUG なしのビルドでは
     * ここで abort していた。読めない 1 枚は捨てて残りを使えば十分なので、
     * 失敗は戻り値 0 で呼び出し側 (httplib) に返すだけにする。 */
...
struct SSL
{
        ...
        : ssl_ctx(ctx)
    {
        ...
        /* --- kbridge patch: 信頼するルート CA を conf に結び付ける --------------
         * 元のシムは mbedtls_ssl_conf_ca_chain() をコメントアウトしていたため、
         * SSL_CTX_get_cert_store()/X509_STORE_add_cert() で積んだシステムルート
         * 証明書がハンドシェイクで一切参照されず、サーバ証明書の検証が必ず失敗した
         * （httplib の enable_server_certificate_verification(true) が使えない）。
         * store->chain は追記されても先頭ノードのアドレスが変わらないので、
         * ここでポインタを渡しておけば以降に追加された CA も有効になる。
         * 変更点は cpp/third_party/PATCHES.md を参照。 */
        if (ssl_ctx->store)
            mbedtls_ssl_conf_ca_chain(&ssl_ctx->conf_, &ssl_ctx->store->chain, nullptr);
