...
/* --- kbridge patch -------------------------------------------------------
 * Windows ビルドでは上で read/write/close を関数マクロにしている。MSVC では
 * 実害が出ないが、mingw(w64devkit) では以降に取り込まれるヘッダの宣言まで
 * 書き換えてしまい winsock2.h の recv/send/closesocket と衝突する。
 * この 3 つのマクロは net_sockets.cpp 内でしか使わないので、ファイル末尾で外す。
